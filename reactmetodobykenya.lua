...
local function strongReact(remoteName)
	_G.Vector = Vector3.new(math.huge, math.huge, math.huge)
	local original
	original = hookmetamethod(game, "__namecall", function(self, ...)
		local args = { ... }
		local method = getnamecallmethod()
		if not checkcaller() and method == "FireServer" and tostring(self) == remoteName then
			for i = 1, 3 do
				self:FireServer(nil, _G.Vector, CFrame.new())
			end
			return nil
		end
...
createButton("Kenya React", 10, function()
	strongReact("Kick")
end)

createButton("Nanito React", 50, function()
	strongReact("Kick")
end)
...
createButton("Cholo React", 90, function()
	...
	originalKeepz = hookmetamethod(game, "__namecall", function(self, ...)
		...
		if not checkcaller() and method == "FireServer" and tostring(self) == "Dribble" then
			for i = 1, 3 do
				self:FireServer(args[1], player.Character.Humanoid:FindFirstChild("LLCL"))
			end
			return nil
		end
...
createButton("Savage React (K)", 130, function()
...
local autoDribbleButton
local function updateAutoDribbleVisual()
	if autoDribbleButton then
		autoDribbleButton.Text = "Auto Dribble: " .. (followBall and "ON" or "OFF")
	end
end

createButton("Auto Dribble (C)", 170, function()
	followBall = not followBall
	updateAutoDribbleVisual()
end)

autoDribbleButton = createButton("Auto Dribble: OFF", 210, function()
	followBall = not followBall
	updateAutoDribbleVisual()
end)
...
runService.RenderStepped:Connect(function()
	if followBall and not isMovingManually and character and character:FindFirstChild("Humanoid") then
